...
kor_eng_tuples = pd.read_csv('input_edited_files/korean_english_tuples_to_memorize.csv').sort_values('Korean')
hangul_hanja_pairs = pd.read_csv('output/hanja_hangul_pairs.csv')

hangul_hanja_english_df = kor_eng_tuples.join(hangul_hanja_pairs.set_index('hangul_word'), on='Korean').sort_values('Korean')

hangul_hanja_english_df.to_csv('output/english_hangul_hanja_tuples_naive.csv', index=False)
